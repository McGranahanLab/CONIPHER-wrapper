findSimpleClusters <- function(phylo.region.list, mut.table, runType = "WES") {
    ######################################################################################################################################################
    ...
    for (cluster in names(UniqCluster)) {
        ...
        for (region in RegionsInCluster) {
            ...
            pyclone.table    <- data.frame(t(sapply(1:nrow(region.mut.table), identify.subclonal.mut.copy.number.ascat, region.mut.table, region.seg.copy, region, sample)), stringsAsFactors = FALSE)
...
findSimpleClustersWithSciClone <- function(phylo.region.list, mut.table, minCCF = 0.05, minDepth = 30, MinClusterSize = 5, CCFmethod = 'phyloCCF', quickCHECK = TRUE, plotting = TRUE, runType = "WES") {
    ######################################################################################################################################################
    ### summary of what's required for function
    ### phylo.region.list - output as part of the initial steps of pyclone script (this contains the CCF of each mutation in each region prior to cluster)
    ### mut.table - the mutation table for the patient
    ### minCCF - what is the minimum CCF to classify a mutation as present in a given region [default 0.05, i.e. estimated as 5% of cancer cells]
    ### minDepth  - what is the minimum depth to use a mutation to determine whether bimodal [default 30 depth]
    ### MinClusterSize - what is the minimum number of mutations for a cluster to be considered [default 5]
    ### CCFmethod - which way of determining CCF do you want to use to determine clsuter [default 'phyloCCF']
    ### quickCHECK - do you want to use sciClone to check for multiple clusters in every region, or if one region suggests there are multiple
    ### clusters, simply move on to next region. If set to TRUE, moves on quickly [default TRUE]
    ### plotting - do you want to plot the sciClone clustering [default TRUE]
    ######################################################################################################################################################
    ...
    ### filter out mutations that are in small clusters
    # MutClusters <- MutClusters[MutClusters %in% names(table(MutClusters)[table(MutClusters) > MinClusterSize])]
    UniqCluster <- unique(MutClusters)  
    ...
    for (cluster in names(UniqCluster)){
        ...
        MutationsWithCluster <- MutClusterNum[MutClusterNum %in% cluster]
        for (region in regionsToUse) {
            ### have we already identified evidence for multiple clusters (if yes, skip provided quickCHECK is TRUE)
            if (quickCHECK) {
                if (ClusterEvidenceForMultiModal[cluster] == 1) {
                    SmallClusters[[cluster]][[region]]$SciCloneMultiClusters <- NA 
                    SmallClusters[[cluster]][[region]]$cluster.assignment    <- NA
                    next
                }
            }

            ### is this cluster present in this region?
            if (sum(BinaryTable[names(MutationsWithCluster), region]) == 0) {
                SmallClusters[[cluster]][[region]]$SciCloneMultiClusters <- NA 
                SmallClusters[[cluster]][[region]]$cluster.assignment    <- NA
                next
            }

            ### perform a quick analysis with SciClone
            regionClusterTable <- phylo.region.list[[region]]
            regionClusterTable <- regionClusterTable[regionClusterTable$mutation_id %in% names(MutationsWithCluster), , drop = FALSE]

            SmallClusters[[cluster]][[region]]$phyloCCF <- unlist(regionClusterTable$phyloCCF)

            sciCloneInput <- sapply(1:nrow(regionClusterTable), getModifiedVAF, regionClusterTable)
            sciCloneInput <- t(sciCloneInput)
            sciCloneInput <- data.frame(sciCloneInput, stringsAsFactors = FALSE)
            colnames(sciCloneInput) <- c('chr', 'pos', 'ref_count', 'var_count', 'vaf')
            sciCloneInput$pos <- as.numeric(sciCloneInput$pos)
            sciCloneInput$ref_count <- as.numeric(sciCloneInput$ref_count)
            sciCloneInput$var_count <- as.numeric(sciCloneInput$var_count)
            sciCloneInput$vaf <- as.numeric(sciCloneInput$vaf) * 100
            if (length(which(sciCloneInput$ref_count + sciCloneInput$var_count > minDepth)) < MinClusterSize) {
                SmallClusters[[cluster]][[region]]$SciCloneMultiClusters <- FALSE 
                SmallClusters[[cluster]][[region]]$cluster.assignment    <- NA
                next
            }

            regionSciCloneOut <- sciClone(sciCloneInput, sampleNames = PasteVector(c(region, "_cluster_", cluster), sep = ""), minimumDepth = minDepth, maximumClusters = 2)
            if(plotting) {
                sc.plot1d(regionSciCloneOut, cnToPlot = c(2))
            }
            if (max(regionSciCloneOut@clust$cluster.assignments) >= 2) {
                ClusterEvidenceForMultiModal[as.character(cluster)] <- 1
                SmallClusters[[cluster]][[region]]$SciCloneMultiClusters <- TRUE 
                SmallClusters[[cluster]][[region]]$cluster.assignment    <- regionSciCloneOut@clust$cluster.assignments
            }
            if (max(regionSciCloneOut@clust$cluster.assignments) < 2) {
                SmallClusters[[cluster]][[region]]$SciCloneMultiClusters <- FALSE 
                SmallClusters[[cluster]][[region]]$cluster.assignment    <- regionSciCloneOut@clust$cluster.assignments
    ...
    # SmallClusters[["ClusterEvidenceForMultiModal"]] <- ClusterEvidenceForMultiModal

    ### next, make pyclone tables so we can also run pyclone on these samples. 
    for (cluster in names(ClusterEvidenceForMultiModal)) {
        ClusterMutationsIDs <- names(MutClusterNum[MutClusterNum %in% cluster])
        specClusterTable <- BinaryTable[ClusterMutationsIDs, , drop = FALSE]
        RegionsInCluster <- colnames(specClusterTable)[which(specClusterTable[1, ] == 1)]
        ClusterTable     <- mut.table[mut.table$mutation_id %in% ClusterMutationsIDs, , drop = FALSE] 
        SmallClusters[[cluster]]$RegionsInCluster <- RegionsInCluster
        if (ClusterEvidenceForMultiModal[cluster] == 0) {
            SmallClusters[[cluster]]$SciCloneMultiModal <- FALSE
        } else {
            SmallClusters[[cluster]]$SciCloneMultiModal <- TRUE    
        }
...
RunPyCloneWithSimpleClusters <- function(clusterName, patientID, SmallClusters, patientDirToUse = new.dir, yamlConfigLoc = template.config.yaml, run.pyclone = TRUE, pyclone.module = "PyClone/0.12.3-foss-2016b-Python-2.7.12-tkinter") {
    ...
    sample.results <- paste0(ClusterDir, "/", sample, '.results.tsv')
    cmd <- paste0(PyClone
