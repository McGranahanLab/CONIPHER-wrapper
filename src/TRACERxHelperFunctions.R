...
determineIndelCorrectionFactor <- function(mut.table, regions.to.use, seg.mat.phylo) {
    ...
    if (length(IndelMuts) == 0) {
        ...
        for (region in regions.to.use) {
            ...
            pyclone.table    <- data.frame(t(sapply(1:nrow(region.mut.table),identify.subclonal.mut.copy.number.ascat,region.mut.table,region.seg.phylo,region,sample)), stringsAsFactors=FALSE)
            ...
            if(!skip.VAFQC) {
                sample.purity   <- read.table(PurityToUse[[region]]
                                            ,sep="\t"
                                            ,header=TRUE
                                            ,stringsAsFactors=FALSE)
                sample.purity   <- as.numeric(sample.purity$VAF.purity)
                if(RegionAction[[region]]%in%"Ignore_VAFQC") {
...
plot.EarlyOrLate.raw    <- function(seg.mat.patient
  ...
  min.y               <- -0.25
  max.y               <- max.cpn +1
  max.x               <- as.numeric(max(seg.mat.plot$EndPosition, na.rm = TRUE))
...
plot.pyclone.clusters <- function(regionList
  ...
  for(i in 1:(length(regionList)-1)){
    for(j in (i+1):length(regionList)){
      ...
      driver.table$driverCategoryNumeric<-as.numeric(revalue(as.character(driver.table$driverCategory), c("1A"=1,"1"=2, "2A"=3, "2"=4, "3"=5))) 
...
plot.pycloneMutCpn.clusters <- function(regionList
                                  ,mostLikelyClusters
                                  ,driverCat
                                  ,ccf = 'sanger'
                                  ,colour.choice = NA
    ...
    for(i in 1:(length(regionList)-1)){
      for(j in (i+1):length(regionList)){
        ...
        if(nrow(driver.table)>1)
        {
          ...
          textplot(unlist(regionX[rownames(driver.table),'mutCopyNum'])
